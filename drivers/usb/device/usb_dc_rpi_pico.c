...
#define RPI_PICO_PINCTRL_DT_INST_DEFINE(n)					\
	COND_CODE_1(DT_INST_PINCTRL_HAS_NAME(n, default),			\
		    (PINCTRL_DT_INST_DEFINE(n)), ())

#define RPI_PICO_PINCTRL_DT_INST_DEV_CONFIG_GET(n)				\
	COND_CODE_1(DT_INST_PINCTRL_HAS_NAME(n, default),			\
		    ((void *)PINCTRL_DT_INST_DEV_CONFIG_GET(n)), (NULL))

RPI_PICO_PINCTRL_DT_INST_DEFINE(0);
const struct pinctrl_dev_config *pcfg = RPI_PICO_PINCTRL_DT_INST_DEV_CONFIG_GET(0);
...
static int udc_rpi_init(void)
{
	...
	/* Self powered devices must enable the pull up only if vbus is
	 * detected. If the pull-up is not enabled here, this will be handled
	 * by the USB_INTS_VBUS_DETECT interrupt.
	 */
	if (usb_hw->sie_status & USB_SIE_STATUS_VBUS_DETECTED_BITS) {
