...
struct usbd_class_api {
	...
	void (*feature_halt)(struct usbd_class_data *const node,
			     uint8_t ep, bool halted);

	/** Configuration update handler */
	void (*update)(struct usbd_class_data *const node,
		       uint8_t iface, uint8_t alternate);

	/** USB control request handler to device */
	int (*control_to_dev)(struct usbd_class_data *const node,
			      const struct usb_setup_packet *const setup,
			      const struct net_buf *const buf);

	/** USB control request handler to host */
	int (*control_to_host)(struct usbd_class_data *const node,
			       const struct usb_setup_packet *const setup,
			       struct net_buf *const buf);

	/** Endpoint request completion event handler */
	int (*request)(struct usbd_class_data *const node,
		       struct net_buf *buf, int err);

	/** USB power management handler suspended */
	void (*suspended)(struct usbd_class_data *const node);

	/** USB power management handler resumed */
	void (*resumed)(struct usbd_class_data *const node);

	/** Start of Frame */
	void (*sof)(struct usbd_class_data *const node);

	/** Class associated configuration is selected */
	void (*enable)(struct usbd_class_data *const node);

	/** Class associated configuration is disabled */
	void (*disable)(struct usbd_class_data *const node);

	/** Initialization of the class implementation */
	int (*init)(struct usbd_class_data *const node);

	/** Shutdown of the class implementation */
	void (*shutdown)(struct usbd_class_data *const node);

	/** Get function descriptor based on speed parameter */
	void *(*get_desc)(struct usbd_class_data *const node,
 ...
 *
 * @param[in] node Pointer to USB device class data
 *
